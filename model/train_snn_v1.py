"""train_snn_v1.py — Spike-time SNN baseline for SCG (256→64→3 FC LIF).

Architecture (mirrors Mirsadeghi et al. 2026 §III but in 1-D and smaller):
  Input:  (B, 256) int8 SCG window
   ↓ direct encoding (x_t = x for all t in [0,T))
  FC1:    256 → H linear, no bias
   ↓ LIF1: v ← β v + I; spike if v ≥ θ; soft reset
  FC2:    H → 3 linear, no bias
...
def main():
    ...
    model = SCGSnn(n_in=256, n_hidden=args.H, n_classes=3,
                   beta=args.beta, threshold=args.threshold, T=args.T).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"SNN 256->{args.H}->3  T={args.T}  beta={args.beta} theta={args.threshold}  "
