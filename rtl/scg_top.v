...
module scg_top #(
    parameter integer CLK_HZ      = 50_000_000,   // HX4S20C on-board 50 MHz crystal
    parameter integer BAUD        = 115_200,
    parameter integer WIN_LEN     = 256,
    parameter integer N_LAYERS    = 4,
    parameter integer WEIGHT_DEPTH = 2048,        // bytes; 2 KB > total 2KB params
    parameter integer ACT_DEPTH   = 4096          // bytes; 2-bank ping-pong
) (
    ...
    reg  [10:0] w_addr_w;                        // write port: driven by FSM
    wire [10:0] w_addr_r;                        // read port:  driven by MAC engine
    reg  [10:0] a_addr_w;                        // write port: driven by FSM (UART loader)
    wire [10:0] a_addr_r;                        // read port:  driven by MAC engine
